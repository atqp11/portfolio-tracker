/**
 * Usage Statistics Client Component
 *
 * Displays user usage statistics, quotas, and tier information.
 * Receives initial data from parent RSC page and provides auto-refresh.
 *
 * Features:
 * - Real-time usage display with progress bars
 * - Auto-refresh every 30 seconds
 * - Warning indicators for approaching limits
 * - Upgrade prompts for users near capacity
 */
...
export default function UsageClient({ initialStats }: UsageClientProps) {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      try {
        const updatedStats = await fetchUsageStats();
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="text-right">
            ...
            <p className="text-xl font-bold text-blue-600 dark:text-blue-400">
              {tierDisplayNames[stats.tier] || stats.tier}
            </p>
            {stats.tier !== 'premium' && (
              <Link
                href="/pricing"
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="text-right">
              <p className="font-semibold text-gray-900 dark:text-gray-100">
                {stats.usage.daily.chatQueries.used} /{' '}
                {formatLimit(stats.usage.daily.chatQueries.limit)}
              </p>
              <p className="text-sm text-gray-600 dark:text-gray-400">
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="text-right">
              <p className="font-semibold text-gray-900 dark:text-gray-100">
                {stats.usage.daily.portfolioAnalysis.used} /{' '}
                {formatLimit(stats.usage.daily.portfolioAnalysis.limit)}
              </p>
              <p className="text-sm text-gray-600 dark:text-gray-400">
          ...
          {stats.usage.daily.portfolioAnalysis.limit !== Infinity && (
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
              <div
                className={`h-2 rounded-full transition-all ${getProgressColor(
                  stats.percentages.portfolioAnalysis
                )}`}
                style={{
                  width: `${Math.min(100, stats.percentages.portfolioAnalysis)}%`,
                }}
              />
            </div>
          )}
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="text-right">
              <p className="font-semibold text-gray-900 dark:text-gray-100">
                {stats.usage.daily.portfolioChanges.used} /{' '}
                {formatLimit(stats.usage.daily.portfolioChanges.limit)}
              </p>
              <p className="text-sm text-gray-600 dark:text-gray-400">
          ...
          {stats.usage.daily.portfolioChanges.limit !== Infinity && (
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
              <div
                className={`h-2 rounded-full transition-all ${getProgressColor(
                  stats.percentages.portfolioChanges
                )}`}
                style={{
                  width: `${Math.min(100, stats.percentages.portfolioChanges)}%`,
                }}
              />
            </div>
          )}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="text-right">
              <p className="font-semibold text-gray-900 dark:text-gray-100">
                {stats.usage.monthly.secFilings.used} /{' '}
                {formatLimit(stats.usage.monthly.secFilings.limit)}
              </p>
              <p className="text-sm text-gray-600 dark:text-gray-400">
      ...
      {hasWarnings && (
        <div className="bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
          ...
          <p className="text-blue-800 dark:text-blue-300 mb-4">
            You're approaching your usage limits. Upgrade to a higher tier for more
