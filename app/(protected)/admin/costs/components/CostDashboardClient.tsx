...
import type { TelemetryStatsDto } from '@backend/modules/telemetry/dto/telemetry.dto';
...
export default function CostDashboardClient({ initialPeriod, initialData }: CostDashboardClientProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [isPending, startTransition] = useTransition();
  const [period, setPeriod] = useState<'1h' | '24h' | '7d' | '30d'>(initialPeriod);
  const [data, setData] = useState(initialData);
  const [loading, setLoading] = useState(false);
  const [autoRefresh, setAutoRefresh] = useState(true);

  // Sync state when server props change (e.g., after period change or refresh triggers server re-render)
  useEffect(() => {
    if (initialPeriod !== period) {
      setPeriod(initialPeriod);
    }
    setData(initialData);
  }, [initialPeriod, initialData, period]);

  const handlePeriodChange = (newPeriod: '1h' | '24h' | '7d' | '30d') => {
    setPeriod(newPeriod);
    // Update URL and trigger server-side re-render (RSC pattern)
    const params = new URLSearchParams(searchParams.toString());
    params.set('period', newPeriod);
    startTransition(() => {
      router.push(`/admin/costs?${params.toString()}`, { scroll: false });
      router.refresh(); // Trigger server-side re-fetch
    });
  };

  // Auto-refresh: Use router.refresh() to trigger server-side re-render (RSC pattern)
  // This calls the service directly on the server, no API route needed
  useEffect(() => {
    if (!autoRefresh) return;

    const interval = setInterval(() => {
      startTransition(() => {
        router.refresh(); // Server-side re-render will fetch fresh data via service
      });
